...
def turning():
    ...
    if f==3:    #E房间
        for i in range(1):
            a=0
            if turn_flag==0:
                print("[ turning ]E第%d次开转！！！"%(i+1))
                while (not rospy.is_shutdown()) and a<50:    #50大概是90度  
                    vel=Twist()
                    vel.linear.x=0.0
                    vel.linear.y=0.0
                    vel.linear.z=0.0
    ...
    if f==4:    #F房间
        ...
        #     if turn_flag==0:
        #         print("[ turning ]F外第%d次开转！！！"%(i+1))
        #         while (not rospy.is_shutdown()) and a<20:    #50大概是90度  
        #             vel=Twist()
        #             vel.linear.x=0.0
        #             vel.linear.y=0.0
        #             vel.linear.z=0.0
...
def img_callback(data):
    ...
    if first_image<=150 and first_image>=0:
        ...
        if first_image >0:
            ...
            goal_pose.header.frame_id = "map" #D点
            goal_pose.pose.position.x = 3.224
            goal_pose.pose.position.y = -4.463
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = 0.678
            goal_pose.pose.orientation.w = 0.735
            goal_pub.publish(goal_pose)
            print("[ img_callback ]发布D点坐标")
            first_image=-1

    if f==1 :
        if img_flag == 0 and skip_D == 0:
            goal_pose = PoseStamped()  
            goal_pose.header.frame_id = "map" #C点里看外
            goal_pose.pose.position.x = 4.711
            goal_pose.pose.position.y = 0.063
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = -0.688
            goal_pose.pose.orientation.w = 0.726
            goal_pub.publish(goal_pose)
        img_flag = 1
    ...
    if f==2 :
        if img_flag == 1 and skip_C == 0:
            goal_pose = PoseStamped()  
            goal_pose.header.frame_id = "map" #B点里看外
            goal_pose.pose.position.x = 5.245
            goal_pose.pose.position.y = -4.795
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = 0.771
            goal_pose.pose.orientation.w = 0.636
            goal_pub.publish(goal_pose)
        img_flag = 2
    ...
    if f==5 :
        if img_flag == 4:
            f+=1
            # goal_pose = PoseStamped()  
            # goal_pose.header.frame_id = "map" #F点A区中心
            # goal_pose.pose.position.x = 1.257
            # goal_pose.pose.position.y = -3.888
            # goal_pose.pose.position.z = 0.000
            # goal_pose.pose.orientation.x = 0
            # goal_pose.pose.orientation.y = 0
            # goal_pose.pose.orientation.z = -0.656
            # goal_pose.pose.orientation.w = 0.754
            # goal_pub.publish(goal_pose)
        img_flag = 5
    ...
    if f==6 :
        if img_flag == 5:
            ...
            # goal_pose.pose.position.x = 0.629
            # goal_pose.pose.position.y = -4.801
            # goal_pose.pose.position.z = 0.000
            # goal_pose.pose.orientation.x = 0
            # goal_pose.pose.orientation.y = 0
            # goal_pose.pose.orientation.z = -0.856
            # goal_pose.pose.orientation.w = 0.517
            # goal_pub.publish(goal_pose)
        img_flag = 6
    ...
    if f==7 :
        if img_flag == 6:
            ...
            goal_pose.pose.position.x = 0.014 #0.010
...
def boxes_callback(msg):
    ...
    if seq==3 and receive_flag==0: #B点内看外
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        b_room.append(slist)
    ...
    if seq>=4 and seq<=5 and receive_flag==0: #E点回头远固定板
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        e_room.append(slist)
    ...
    if seq>=6 and seq<=8 and receive_flag==0: #F点外三张照片
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        f_room.append(slist)
    ...
    if seq>=9 and seq<=12 and receive_flag==0: #F点A四张照片
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        f_room.append(slist)
    ...
    if seq==13 and receive_flag==0: #F点内一张照片
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        f_room.append(slist)
