...
def img_callback(data):
    ...
    if f==1 :
        if img_flag == 0 and skip_D == 0:
            ...
            goal_pose.header.frame_id = "map" #C点对角左
            goal_pose.pose.position.x = 5.232
            goal_pose.pose.position.y = -1.214
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = 0.890
            goal_pose.pose.orientation.w = 0.456
    ...
    if f==2 :
        if img_flag == 1 and skip_C == 0:
            goal_pose = PoseStamped()  
            goal_pose.header.frame_id = "map" #B点右
            goal_pose.pose.position.x = 5.647
            goal_pose.pose.position.y = -3.087
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = 1.000
            goal_pose.pose.orientation.w = -0.013
            goal_pub.publish(goal_pose)
        img_flag = 2
...
def boxes_callback(msg):
    ...
    if seq==2 and receive_flag==0:
        ...
        c_room.append(slist)
        room_class(c_room)
        # if c_room[0]!=0 and img_flag==1: #C点对角左
        #     skip_C=1
        #     goal_pose = PoseStamped()
        #     goal_pose.header.frame_id = "map" #B点右
        #     goal_pose.pose.position.x = 5.647
    ...
    if seq==3 and receive_flag==0: #B点右
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        b_room.append(slist)
