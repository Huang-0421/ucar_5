...
def img_callback(data):
    ...
    if f==2 :
        if img_flag == 1 and skip_C == 0:
            goal_pose = PoseStamped()  
            goal_pose.header.frame_id = "map" #B点对角右
            goal_pose.pose.position.x = 5.493
            goal_pose.pose.position.y = -3.114
            goal_pose.pose.position.z = 0.000
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
...
def boxes_callback(msg):
    ...
    if seq==3 and receive_flag==0: #B点对角右
        slist=[]
        for i in range(len(msg.bounding_boxes)):
            slist.append(msg.bounding_boxes[i].Class)
        b_room.append(slist)
