...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    classes_num.corn_num = 0;
    classes_num.cucumber_num = 0;
    classes_num.rice_num = 0;//xiao
    classes_num.wheat_num = 0;
    classes_num.corn_cut_num = 0;
    classes_num.cucumber_cut_num = 0;
    classes_num.rice_cut_num = 0;//xiao
    classes_num.wheat_cut_num = 0;
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          if (classLabels_[i]=="corn" )
        {
          classes_num.corn_num++;
          if (xmax > 340)
          {
            classes_num.corn_cut_num++; 
          }
        }  
        else if(classLabels_[i]=="cucumber")
        {
          classes_num.cucumber_num++;
          if (xmax > 340)
          {
            classes_num.cucumber_cut_num++; 
          }
        }
        else if(classLabels_[i]=="rice")//xiaos
        {
          classes_num.rice_num++;
          if(xmax>340)
          {
            classes_num.rice_cut_num++;
          }
        }
        else if(classLabels_[i]=="wheat")
        {
          classes_num.wheat_num++;
          if (xmax > 340)
          {
            classes_num.wheat_cut_num++; 
          }
        }
        }
      }
