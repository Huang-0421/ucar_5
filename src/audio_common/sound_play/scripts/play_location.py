...
def point_filter(resp):
    ...
    for i  in range(len(resp.cloud.points)):
        ...
        if 4.6<x<5.48 and -5.43<y<-2.55 and f==1:
            point=[x,y]
            point_list.append(point)

        if 2.6<x<3.7 and -5.43<y<-3.8 and f==4:
            point=[x,y]
            point_list.append(point)

        if -0.12<x<2.01 and -5.44<y<-2.8 and f==7:
            point=[x,y]
            point_list.append(point)
...
def boxes_callback(msg):
    global seq,b_room,c_room,d_room,f,img_flag,error_image_num,fang3
    ...
    if seq>=5 and seq<=7:
        ...
        c_room.append(slist)
        room_class(c_room)
        if c_room[0]!=0 and b_room[0]!=0 and fang3==0:
...
def img_callback(data):
    global f,img_flag,Img_publish,fang,b_room,c_room,d_room,first_image,flag_3,fang3
...
def give_goal(result_list):
    ...
    if f==5:
        ...
        goal_pose.pose.orientation.x = 0
        goal_pose.pose.orientation.y = 0
        goal_pose.pose.orientation.z = result_list[ii][0]
        goal_pose.pose.orientation.w = result_list[ii][1]
        if ii==2:
            ii=1
...
def goal_callback(msg):
    ...
    if f==1 or f==4 or f==7:
        ...
        if f==4:
            for i in range(len(ThePoint)):
                result=calculate_direction([0,0],[ThePoint[i,0],ThePoint[i,1]])
                if result[2]==10 or result[2]==11 or result[2]==12 or result[2]==13 or result[2]==14:
                    result_list.append([-0.707,0.707,6])
                else:
                    result_list.append([-0.707,0.707,6])
                    result_list.append(result)
            print(result_list)
