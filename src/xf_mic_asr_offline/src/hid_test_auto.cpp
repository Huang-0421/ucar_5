...
bool pubCommand = false; //chushipanduan
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("seconds_per_order", time_per_order, 5); //单次录制音频的时长
	...
	goal_pub = ndHandle.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal",1);
	...
	/*send goal*/
	ros::ServiceServer command_service = ndHandle.advertiseService("/send_goal_srv",commandCallback);
