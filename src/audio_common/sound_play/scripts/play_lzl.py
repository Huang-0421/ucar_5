...
def room_sound():
    ...
    if  b_room[0]==1 and c_room[0]==1 and  d_room[0]==1:
        ...
        fang ="b2c3d3"
    if  b_room[0]==3 and c_room[0]==1 and  d_room[0]==1:
        fang ="b3c1d1"
...
def turning():
    global Img_publish,img_pub,img_num,run_flag
    ...
    for i in range(4):
        ...
        a=0
        print("开转！！！")
        while (not rospy.is_shutdown()) and a<10:    #16大概是90度  
            vel=Twist()
            vel.linear.x=0.0
            vel.linear.y=0.0
            vel.linear.z=0.0
...
def img_callback(data):
    ...
    if f==1 :
        if img_flag == 0:
            ...
            goal_pose.pose.position.x = 3.07
            goal_pose.pose.position.y = -4.25
            goal_pose.pose.position.z = 0.0
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z =  -0.993
        ...
        if img_flag == 1:
            img_flag = 2
            Img_publish = data            #第三个房间第一个方向
            goal_pose.header.frame_id = "map"
            goal_pose.pose.position.x = 1.086
            goal_pose.pose.position.y = -3.950
            goal_pose.pose.position.z = 0.0
            goal_pose.pose.orientation.x = 0
            goal_pose.pose.orientation.y = 0
            goal_pose.pose.orientation.z = 0.99
            goal_pose.pose.orientation.w = -0.000874
            goal_pub.publish(goal_pose)
...
def boxes_callback(msg):
    global seq,b_room,c_room,d_room,f,img_flag,error_image_num,fang3,fang33,flag_3,goal_pose,error_image_num,run_flag
...
def goal_callback(msg):
    #更新小车位置信息
    global f,img_num,img_pub,Img_publish
    print("fffffffffffff")
    print(f)
