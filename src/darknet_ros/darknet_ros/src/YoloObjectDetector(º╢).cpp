...
void YoloObjectDetector::init() {
  ...
  classes_num_pub_ =
      nodeHandle_.advertise<darknet_ros_msgs::classes>("classes_num",10);
  // Action servers.
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    classes_num.tableware_num = 0;
    classes_num.pet_num = 0;
    classes_num.bed_num = 0;//xiao
    classes_num.television_num = 0;
    classes_num.person_num = 0;//xiao
    classes_num.sofa_num =0;
    classes_num.food_num =0;
    classes_num.table_chair_num =0;
    classes_num.tableware_cut_num = 0;
    classes_num.pet_cut_num = 0;
    classes_num.bed_cut_num = 0;//xiao
    classes_num.television_cut_num = 0;
    classes_num.person_cut_num = 0;//xiao
    classes_num.sofa_cut_num =0;
    classes_num.food_cut_num =0;
    classes_num.table_chair_cut_num =0;
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          if (classLabels_[i]=="tableware" )
        {
          classes_num.tableware_num++;
          if (xmax > 340)
          {
            classes_num.tableware_cut_num++; 
          }
        }  
        else if(classLabels_[i]=="pet")
        {
          classes_num.pet_num++;
          if (xmax > 340)
          {
            classes_num.pet_cut_num++; 
          }
        }
        else if(classLabels_[i]=="bed")//xiaos
        {
          classes_num.bed_num++;
          if(xmax>340)
          {
            classes_num.bed_cut_num++;
          }
        }
        else if(classLabels_[i]=="television")
        {
          classes_num.television_num++;
          if (xmax > 340)
          {
            classes_num.television_cut_num++; 
          }
        }
        else if(classLabels_[i]=="person")
        {
          classes_num.person_num++;
          if (xmax > 340)
          {
            classes_num.person_cut_num++; 
          }
        }
        else if(classLabels_[i]=="sofa")
        {
          classes_num.sofa_num++;
          if (xmax > 340)
          {
            classes_num.sofa_cut_num++; 
          }
        }
        else if(classLabels_[i]=="food")
        {
          classes_num.food_num++;
          if (xmax > 340)
          {
            classes_num.food_cut_num++; 
          }
        }
        else if(classLabels_[i]=="table/chair")
        {
          classes_num.table_chair_num++;
          if (xmax > 340)
          {
            classes_num.table_chair_cut_num++; 
          }
        }
        }
    ...
    classes_num_pub_.publish(classes_num);
  } else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
    msg.header.frame_id = "detection";
